...
export default function ResultsView() {
  ...
  const fetchBatchesWithFiles = async () => {
    ...
    try {
      // 获取所有批次
      const batchesRes = await axios.get('http://localhost:5001/api/batches')
  ...
  const fetchFileContent = async (filename: string, category: string, batchId: string) => {
    ...
    try {
      ...
      if (category === 'uploaded') {
        endpoint = `http://localhost:5001/api/file-content/uploaded/${encodedFilename}`
      } else if (category === 'processed') {
        endpoint = `http://localhost:5001/api/file-content/processed/${encodedFilename}`
      } else {
        endpoint = `http://localhost:5001/api/file-content/llm-processed/${encodedFilename}`
      }
